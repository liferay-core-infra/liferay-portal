...
public class CPDefinitionVirtualSettingPersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		...
		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<CPDefinitionVirtualSetting>> resultMap =
				new HashMap<>();
			...
			for (CPDefinitionVirtualSetting cpDefinitionVirtualSetting :
				...
				if (Objects.equals(
					...
					List<CPDefinitionVirtualSetting> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(cpDefinitionVirtualSetting);
				}
			}

			for (Map.Entry<List<Object>, List<CPDefinitionVirtualSetting>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<CPDefinitionVirtualSetting> value = resultEntry.getValue();
