...
public class ContainerLPKGUtilTest {
	...
	public void testDeployWithZipSlip() throws Exception {
		...
		File lpkgContainerFile = _createLPKGContainerFile(
			testPath, "out.lpkg", "inner.lpkg", "../inner2.lpkg");
		...
		try (LogCapture logCapture = LoggerTestUtil.configureJDKLogger(
			...
			Assert.assertTrue(Files.exists(testPath.resolve("inner.lpkg")));
			Assert.assertFalse(
				Files.exists(parentOfTestPath.resolve("inner2.lpkg")));
			...
			Assert.assertEquals(
				"Invalid LPKG File name: ../inner2.lpkg",
				logEntry.getMessage());
		}
		finally {
			_fileImpl.deltree(testPath.toFile());
