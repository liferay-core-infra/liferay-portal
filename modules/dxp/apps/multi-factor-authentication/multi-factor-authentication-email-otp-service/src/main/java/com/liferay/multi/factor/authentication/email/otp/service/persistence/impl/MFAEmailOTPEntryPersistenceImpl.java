...
public class MFAEmailOTPEntryPersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<MFAEmailOTPEntry> mfaEmailOTPEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<MFAEmailOTPEntry>> resultMap =
				new HashMap<>();

			for (MFAEmailOTPEntry mfaEmailOTPEntry : mfaEmailOTPEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						mfaEmailOTPEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), mfaEmailOTPEntry);
				}
				else {
					List<MFAEmailOTPEntry> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(mfaEmailOTPEntry);
				}
			}

			for (Map.Entry<List<Object>, List<MFAEmailOTPEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<MFAEmailOTPEntry> value = resultEntry.getValue();
