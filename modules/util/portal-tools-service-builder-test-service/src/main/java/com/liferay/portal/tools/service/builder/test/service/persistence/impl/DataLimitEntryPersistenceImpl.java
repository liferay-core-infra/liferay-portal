...
public class DataLimitEntryPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<DataLimitEntry> dataLimitEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<DataLimitEntry>> resultMap = new HashMap<>();

			for (DataLimitEntry dataLimitEntry : dataLimitEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						dataLimitEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), dataLimitEntry);
				}
				else {
					List<DataLimitEntry> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(dataLimitEntry);
				}
			}

			for (Map.Entry<List<Object>, List<DataLimitEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<DataLimitEntry> value = resultEntry.getValue();
