...
public class SXPBlueprintPersistenceImpl
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(
			_finderPathFetchByC_ERC, args, sxpBlueprintModelImpl);
	}
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<SXPBlueprint> sxpBlueprints = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<SXPBlueprint>> resultMap = new HashMap<>();
			...
			for (SXPBlueprint sxpBlueprint : sxpBlueprints) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						sxpBlueprintModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), sxpBlueprint);
				}
				else {
					List<SXPBlueprint> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(sxpBlueprint);
				}
			}

			for (Map.Entry<List<Object>, List<SXPBlueprint>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<SXPBlueprint> value = resultEntry.getValue();
