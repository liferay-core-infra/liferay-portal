...
public class PortalImplCanonicalURLTest {
	...
	@Test
	@TestInfo("LPD-98055")
	public void testCanonicalURLLegacyLayoutFriendlyURLWithNonascii()
		throws Exception {
	...
	public void testCanonicalURLVirtualLayout() throws Exception {
		...
		ThemeDisplay themeDisplay = _createThemeDisplay(
			"localhost", _targetGroup, 8080, false);

		String completeURL = _generateURL(
			"localhost", "8080", StringPool.BLANK,
			_targetGroup.getFriendlyURL(), virtualLayout.getFriendlyURL(),
			false);

		Assert.assertEquals(
			_generateURL(
				"localhost", "8080", StringPool.BLANK,
				_targetGroup.getFriendlyURL(), StringPool.BLANK, false),
			_portal.getCanonicalURL(
				completeURL, themeDisplay, virtualLayout, false, false));
		Assert.assertEquals(
			completeURL,
			_portal.getCanonicalURL(
				completeURL, themeDisplay, virtualLayout, true, false));
	}
