...
public class IndexResourceImpl extends BaseIndexResourceImpl {
	...
	private static final Map<String, String> _indexNameSuffixMap =
		HashMapBuilder.put(
			"instance", WorkflowMetricsIndexEntityNameConstants.POSTFIX_INSTANCE
		).put(
			"node", WorkflowMetricsIndexEntityNameConstants.POSTFIX_NODE
		).put(
			"process", WorkflowMetricsIndexEntityNameConstants.POSTFIX_PROCESS
		).put(
			"sla-instance-result",
			WorkflowMetricsIndexEntityNameConstants.POSTFIX_SLA_INSTANCE_RESULT
		).put(
			"sla-task-result",
			WorkflowMetricsIndexEntityNameConstants.POSTFIX_SLA_TASK_RESULT
		).put(
			"task", WorkflowMetricsIndexEntityNameConstants.POSTFIX_TASK
		).put(
			"transition",
			WorkflowMetricsIndexEntityNameConstants.POSTFIX_TRANSITION
		).build();
