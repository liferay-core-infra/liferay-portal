...
public class ESHeapSizeRuleImplTest {
	...
	public void testCheckReturnsFail() {
		Collection<Result> results = _esHeapSizeRuleImpl.check(0L);
		...
		Assert.assertEquals(
			"ES_JAVA_OPTS -Xms16g", result.getRecommendedValue());
		Assert.assertEquals(0, result.getMessageParameters().length);
	}

	private final ESHeapSizeRuleImpl _esHeapSizeRuleImpl =
		new ESHeapSizeRuleImpl();
