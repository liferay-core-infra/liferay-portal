...
public class CTProcessPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<CTProcess> ctProcesss = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<CTProcess>> resultMap = new HashMap<>();

			for (CTProcess ctProcess : ctProcesss) {
				List<Object> arguments = new ArrayList<>();

				for (String columnName : finderPath.getColumnNames()) {
					CTProcessModelImpl ctProcessModelImpl =
						(CTProcessModelImpl)ctProcess;

					arguments.add(
						ctProcessModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), ctProcess);
				}
				else {
					List<CTProcess> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(ctProcess);
				}
			}

			for (Map.Entry<List<Object>, List<CTProcess>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<CTProcess> value = resultEntry.getValue();
