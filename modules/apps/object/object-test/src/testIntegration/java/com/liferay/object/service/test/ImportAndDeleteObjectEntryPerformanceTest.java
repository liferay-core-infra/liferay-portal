...
public class ImportAndDeleteObjectEntryPerformanceTest {
	...
	public static void setUpClass() throws Exception {
		_objectFolderJSON = StringUtil.read(
			ImportAndDeleteObjectEntryPerformanceTest.class.getClassLoader(),
	...
	private void _deleteObjectEntry() {
		try (LoggingTimer loggingTimer = new LoggingTimer(
			...
			while (_currentObjectEntryCount != 0) {
				_currentObjectEntryCount =
	...
	private void _importObjectEntry() {
		try (LoggingTimer loggingTimer = new LoggingTimer(
			...
			_currentObjectEntryCount = 0;

			while (_currentObjectEntryCount < _OBJECT_ENTRY_COUNT) {
				_currentObjectEntryCount =
					_objectEntryLocalService.getObjectEntriesCount(
						_objectDefinition.getObjectDefinitionId());
			}
	...
	private long _currentObjectEntryCount;
