 ...
 * @generated
 */
@Component(service = {KBFolderPersistence.class, BasePersistence.class})
public class KBFolderPersistenceImpl
	...
	public void activate() {
		...
		_finderPathFetchByG_P_UT = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByG_P_UT",
			new String[] {
				Long.class.getName(), Long.class.getName(),
				String.class.getName()
			},
		...
		_finderPathCountByG_P_UT = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByG_P_UT",
			new String[] {
				Long.class.getName(), Long.class.getName(),
				String.class.getName()
			},
		...
		_finderPaths.put("finderPathFetchByG_P_UT", _finderPathFetchByG_P_UT);

		_finderPaths.put("finderPathCountByG_P_UT", _finderPathCountByG_P_UT);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<KBFolder> kbFolders = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<KBFolder>> resultMap = new HashMap<>();
			...
			for (KBFolder kbFolder : kbFolders) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					KBFolderModelImpl kbFolderModelImpl =
						(KBFolderModelImpl)kbFolder;

					arguments.add(kbFolderModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), kbFolder);
				}
				else {
					List<KBFolder> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(kbFolder);
				}
			}

			for (Map.Entry<List<Object>, List<KBFolder>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<KBFolder> value = resultEntry.getValue();
