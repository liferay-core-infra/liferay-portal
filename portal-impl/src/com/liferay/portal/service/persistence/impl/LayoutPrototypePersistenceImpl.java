...
public class LayoutPrototypePersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<LayoutPrototype> layoutPrototypes = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<LayoutPrototype>> resultMap =
				new HashMap<>();
			...
			for (LayoutPrototype layoutPrototype : layoutPrototypes) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						layoutPrototypeModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					FinderCacheUtil.putResult(
						finderPath, arguments.toArray(), layoutPrototype);
				}
				else {
					List<LayoutPrototype> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(layoutPrototype);
				}
			}

			for (Map.Entry<List<Object>, List<LayoutPrototype>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<LayoutPrototype> value = resultEntry.getValue();
