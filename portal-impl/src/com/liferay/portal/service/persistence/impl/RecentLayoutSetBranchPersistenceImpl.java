...
public class RecentLayoutSetBranchPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathFetchByU_L = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByU_L",
			new String[] {Long.class.getName(), Long.class.getName()},
			new String[] {"userId", "layoutSetId"}, true);

		_finderPathCountByU_L = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByU_L",
			new String[] {Long.class.getName(), Long.class.getName()},
		...
		_finderPaths.put("finderPathFetchByU_L", _finderPathFetchByU_L);

		_finderPaths.put("finderPathCountByU_L", _finderPathCountByU_L);

		_setRecentLayoutSetBranchUtilPersistence(this);
	}

	public void destroy() {
		_setRecentLayoutSetBranchUtilPersistence(null);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<RecentLayoutSetBranch> recentLayoutSetBranchs = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<RecentLayoutSetBranch>> resultMap =
				new HashMap<>();
			...
			for (RecentLayoutSetBranch recentLayoutSetBranch :
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						recentLayoutSetBranchModelImpl.getColumnValue(
							columnName));
				}
				...
				if (Objects.equals(
					...
					FinderCacheUtil.putResult(
						finderPath, arguments.toArray(), recentLayoutSetBranch);
				}
				else {
					List<RecentLayoutSetBranch> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(recentLayoutSetBranch);
				}
			}

			for (Map.Entry<List<Object>, List<RecentLayoutSetBranch>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<RecentLayoutSetBranch> value = resultEntry.getValue();
	...
	private void _setRecentLayoutSetBranchUtilPersistence(
		RecentLayoutSetBranchPersistence recentLayoutSetBranchPersistence) {

		try {
			Field field = RecentLayoutSetBranchUtil.class.getDeclaredField(
				"_persistence");

			field.setAccessible(true);

			field.set(null, recentLayoutSetBranchPersistence);
		}
