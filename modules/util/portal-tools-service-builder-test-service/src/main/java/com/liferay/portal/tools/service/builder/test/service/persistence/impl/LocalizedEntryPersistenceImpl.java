...
public class LocalizedEntryPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<LocalizedEntry> localizedEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<LocalizedEntry>> resultMap = new HashMap<>();

			for (LocalizedEntry localizedEntry : localizedEntrys) {
				List<Object> arguments = new ArrayList<>();

				for (String columnName : finderPath.getColumnNames()) {
					LocalizedEntryModelImpl localizedEntryModelImpl =
						(LocalizedEntryModelImpl)localizedEntry;

					arguments.add(
						localizedEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), localizedEntry);
				}
				else {
					List<LocalizedEntry> resultList = resultMap.computeIfAbsent(
			...
			for (Map.Entry<List<Object>, List<LocalizedEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<LocalizedEntry> value = resultEntry.getValue();
