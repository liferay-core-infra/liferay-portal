...
public class AppPersistenceImpl
	...
	public void activate() {
		...
		_finderPathFetchByRemoteAppId = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByRemoteAppId",
		...
		_finderPathCountByRemoteAppId = new FinderPath(
			...
			new String[] {"remoteAppId"}, false);

		_finderPathWithPaginationFindByCategory = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByCategory",
			new String[] {
				String.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			},
		...
		_finderPathWithoutPaginationFindByCategory = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByCategory",
			new String[] {String.class.getName()}, new String[] {"category"},
			true);

		_finderPathCountByCategory = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByCategory",
			new String[] {String.class.getName()}, new String[] {"category"},
			false);
		...
		_finderPaths.put(
			"finderPathFetchByRemoteAppId", _finderPathFetchByRemoteAppId);

		_finderPaths.put(
			"finderPathCountByRemoteAppId", _finderPathCountByRemoteAppId);

		_finderPaths.put(
			"finderPathWithPaginationFindByCategory",
			_finderPathWithPaginationFindByCategory);

		_finderPaths.put(
			"finderPathWithoutPaginationFindByCategory",
			_finderPathWithoutPaginationFindByCategory);

		_finderPaths.put(
			"finderPathCountByCategory", _finderPathCountByCategory);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		...
		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<App>> resultMap = new HashMap<>();
			...
			for (App app : apps) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(appModelImpl.getColumnValue(columnName));
				}

				if (Objects.equals(
						finderPath.getCacheName(), FINDER_CLASS_NAME_ENTITY)) {

					finderCache.putResult(finderPath, arguments.toArray(), app);
				}
				else {
					List<App> resultList = resultMap.computeIfAbsent(
			...
			for (Map.Entry<List<Object>, List<App>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<App> value = resultEntry.getValue();
