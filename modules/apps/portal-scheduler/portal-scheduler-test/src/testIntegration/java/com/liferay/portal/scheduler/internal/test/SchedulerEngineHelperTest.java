...
public class SchedulerEngineHelperTest {
	...
	public void testJobIsNotScheduledIfCronExpressionIsBlank()
		...
		_assertExpectedLogs(
			() -> {
				try (AutoCloseable autoCloseable1 = _swapSchedulerEngine()) {
					try (AutoCloseable autoCloseable2 =
							_registerCronJobConfiguration(StringPool.BLANK)) {
			...
			new ExpectedLogs(
				LoggerTestUtil.ERROR, _LOG_CLASS,
	...
	private void _assertExpectedLogs(
			UnsafeRunnable<Exception> runnable, ExpectedLogs expectedLogs)
		throws Exception {

		try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
				expectedLogs._loggerClassName, expectedLogs._level)) {
			...
			for (LogEntry logEntry : logEntries) {
				Assert.assertEquals(
					expectedLogs._level, logEntry.getPriority());

				Assert.assertTrue(
					ArrayUtil.contains(
						expectedLogs._logMessages, logEntry.getMessage()));

				Throwable throwable = logEntry.getThrowable();

				Assert.assertTrue(
					ArrayUtil.contains(
						expectedLogs._logMessages, throwable.getMessage()));
			}
		}
	}
	...
	private static class ExpectedLogs {

		public ExpectedLogs(
			String level, String loggerClassName, String... logMessages) {
		...
		private final String _loggerClassName;
		private final String[] _logMessages;
