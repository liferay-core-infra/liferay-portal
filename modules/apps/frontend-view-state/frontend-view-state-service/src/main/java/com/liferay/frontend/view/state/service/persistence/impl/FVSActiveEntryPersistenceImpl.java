...
public class FVSActiveEntryPersistenceImpl
	...
	public void activate() {
		...
		_finderPathFetchByU_CDSDI_P_P = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByU_CDSDI_P_P",
			new String[] {
		...
		_finderPaths.put(
			"finderPathFetchByU_CDSDI_P_P", _finderPathFetchByU_CDSDI_P_P);

		_finderPaths.put(
			"finderPathCountByU_CDSDI_P_P", _finderPathCountByU_CDSDI_P_P);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<FVSActiveEntry> fvsActiveEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<FVSActiveEntry>> resultMap = new HashMap<>();

			for (FVSActiveEntry fvsActiveEntry : fvsActiveEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						fvsActiveEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), fvsActiveEntry);
				}
				else {
					List<FVSActiveEntry> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(fvsActiveEntry);
				}
			}

			for (Map.Entry<List<Object>, List<FVSActiveEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<FVSActiveEntry> value = resultEntry.getValue();
