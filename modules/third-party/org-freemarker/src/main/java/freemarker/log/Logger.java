...
public abstract class Logger {
	 ...
	 *
	 * @deprecated As of Cavanaugh (7.4.x), avalon LogKit support will be removed sometimes in the future
	 */
	@Deprecated
	 ...
	 *
	 * @deprecated As of Cavanaugh (7.4.x), This method isn't reliable, unless you can somehow ensure that you access the FreeMarker classes
	...
	public static void selectLoggerLibrary(int libraryEnum)
		...
		if ((libraryEnum < MIN_LIBRARY_ENUM) ||
			(libraryEnum > MAX_LIBRARY_ENUM)) {

			throw new IllegalArgumentException(
				"Library enum value out of range");
		}

		synchronized (Logger.class) {
			boolean loggerFactoryWasAlreadySet = false;

			if (loggerFactory != null) {
				loggerFactoryWasAlreadySet = true;
			}

			if (!loggerFactoryWasAlreadySet ||
				(libraryEnum != Logger.libraryEnum)) {
	 ...
	 *
	 * @deprecated As of Cavanaugh (7.4.x), This wasn't reliable, unless you can somehow ensure that you access the FreeMarker classes first. As
	 ...
	 */
	private static LoggerFactory createLoggerFactory(int libraryEnum)
		throws ClassNotFoundException {

		if (libraryEnum == LIBRARY_AUTO) {
			for (int libraryEnumToTry = MAX_LIBRARY_ENUM;
				 libraryEnumToTry >= MIN_LIBRARY_ENUM; libraryEnumToTry--) {

				if (!isAutoDetected(libraryEnumToTry))

					continue;

				if ((libraryEnumToTry == LIBRARY_LOG4J) &&
					hasLog4LibraryThatDelegatesToWorkingSLF4J()) {
			...
			logWarnInLogger(
				"Auto detecton couldn't set up any logger libraries; FreeMarker logging suppressed.");
	 ...
	 */
	private static LoggerFactory createLoggerFactoryForNonAuto(int libraryEnum)
		throws ClassNotFoundException {

		String availabilityCheckClassName = getAvailabilityCheckClassName(
			libraryEnum);
		...
		if (availabilityCheckClassName != null) { // Dynamically created factory
			...
			try {
				return (LoggerFactory)Class.forName(
					"freemarker.log._" + libraryName + "LoggerFactory"
				).newInstance();
			}
			catch (Exception e) {
				throw new RuntimeException(
					"Unexpected error when creating logger factory for \"" +
						libraryName + "\".",
					e);
			}
		}
	...
	private static void ensureLoggerFactorySet(
		...
		synchronized (Logger.class) {
			...
			String sysPropVal = getSystemProperty(
				SYSTEM_PROPERTY_NAME_LOGGER_LIBRARY);
			...
			if (sysPropVal != null) {
				...
				do {
					if (sysPropVal.equalsIgnoreCase(
							getLibraryName(matchedEnum))) {
			...
			try {
				...
				logErrorInLogger(
					"Couldn't set up logger for \"" +
						getLibraryName(libraryEnum) + "\"" +
							(disableLogging ? "; logging disabled" : "."),
					e);
	...
	private static String getSystemProperty(final String key) {
		try {
			...
			logWarnInLogger(
				"Insufficient permissions to read system property \"" + key +
					"\".");
			...
			logErrorInLogger(
				"Failed to read system property \"" + key + "\".", e);
	...
	private static boolean hasLog4LibraryThatDelegatesToWorkingSLF4J() {
		...
		try {
			...
			try {
				Object r = Class.forName(
					LOG4J_OVER_SLF4J_TESTER_CLASS
				).getMethod(
					"test", new Class<?>[0]
				).invoke(
					null
	...
	private static boolean isAutoDetected(int libraryEnum) {
		...
		if (!((libraryEnum == LIBRARY_AUTO) || (libraryEnum == LIBRARY_NONE) ||
			  (libraryEnum == LIBRARY_SLF4J) ||
			  (libraryEnum == LIBRARY_COMMONS))) {
	...
	private static void logInLogger(
		...
		synchronized (Logger.class) {
			canUseRealLogger =
				(loggerFactory != null) &&
				!(loggerFactory instanceof _NullLoggerFactory);
		}
		...
		if (!canUseRealLogger) {
			...
			if (exception != null) {
				...
				while (exception.getCause() != null) {
					...
					System.err.println(
						"\tCaused by: " + tryToString(exception));
				}
			}
		}
	}
	...
	private static synchronized void setLibrary(int libraryEnum)
		throws ClassNotFoundException {
