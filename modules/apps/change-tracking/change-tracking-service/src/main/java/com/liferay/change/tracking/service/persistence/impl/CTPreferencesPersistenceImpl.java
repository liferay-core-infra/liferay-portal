...
public class CTPreferencesPersistenceImpl
	...
	public void activate() {
		...
		_finderPaths.put(
			"finderPathWithPaginationFindByPreviousCtCollectionId",
			_finderPathWithPaginationFindByPreviousCtCollectionId);

		_finderPaths.put(
			"finderPathWithoutPaginationFindByPreviousCtCollectionId",
			_finderPathWithoutPaginationFindByPreviousCtCollectionId);

		_finderPaths.put(
			"finderPathCountByPreviousCtCollectionId",
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<CTPreferences> ctPreferencess = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<CTPreferences>> resultMap = new HashMap<>();

			for (CTPreferences ctPreferences : ctPreferencess) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						ctPreferencesModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), ctPreferences);
				}
				else {
					List<CTPreferences> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(ctPreferences);
				}
			}

			for (Map.Entry<List<Object>, List<CTPreferences>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<CTPreferences> value = resultEntry.getValue();
