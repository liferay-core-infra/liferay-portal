...
public class TestEntityPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<TestEntity> testEntitys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<TestEntity>> resultMap = new HashMap<>();
			...
			for (TestEntity testEntity : testEntitys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					TestEntityModelImpl testEntityModelImpl =
						(TestEntityModelImpl)testEntity;

					arguments.add(
						testEntityModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), testEntity);
				}
				else {
					List<TestEntity> resultList = resultMap.computeIfAbsent(
			...
			for (Map.Entry<List<Object>, List<TestEntity>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<TestEntity> value = resultEntry.getValue();
