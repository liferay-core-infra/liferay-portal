...
public class CTSchemaVersionPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<CTSchemaVersion> ctSchemaVersions = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<CTSchemaVersion>> resultMap =
				new HashMap<>();
			...
			for (CTSchemaVersion ctSchemaVersion : ctSchemaVersions) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						ctSchemaVersionModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), ctSchemaVersion);
				}
				else {
					List<CTSchemaVersion> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(ctSchemaVersion);
				}
			}

			for (Map.Entry<List<Object>, List<CTSchemaVersion>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<CTSchemaVersion> value = resultEntry.getValue();
