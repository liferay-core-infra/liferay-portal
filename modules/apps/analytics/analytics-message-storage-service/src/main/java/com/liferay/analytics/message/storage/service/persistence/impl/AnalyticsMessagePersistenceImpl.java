...
public class AnalyticsMessagePersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<AnalyticsMessage> analyticsMessages = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<AnalyticsMessage>> resultMap =
				new HashMap<>();
			...
			for (AnalyticsMessage analyticsMessage : analyticsMessages) {
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), analyticsMessage);
				}
				else {
					List<AnalyticsMessage> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(analyticsMessage);
				}
			}

			for (Map.Entry<List<Object>, List<AnalyticsMessage>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<AnalyticsMessage> value = resultEntry.getValue();
