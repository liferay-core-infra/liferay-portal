...
public class CompanyLogServletTest {
	...
	private void _assertCompanyLogFiles(
		...
		Assert.assertTrue(
			"Response content should include companyId " +
				company.getCompanyId(),
			companyLogFileJSONObject.getLong("companyId") ==
				company.getCompanyId());
		Assert.assertEquals(
			"Response content should include webId " + company.getWebId(),
		...
		for (File file : companyLogFiles) {
			JSONObject companyLogFileInfoJSONObject =
				companyLogFileInfosJSONArray.getJSONObject(index++);

			String expectedLogFileSize = _language.formatStorageSize(
				file.length(), mockHttpServletRequest.getLocale());

			Assert.assertEquals(
				"Log file name should be " + file.getName(), file.getName(),
				companyLogFileInfoJSONObject.getString("logFileName"));
			Assert.assertEquals(
				"Log file size should be  " + expectedLogFileSize,
				expectedLogFileSize,
