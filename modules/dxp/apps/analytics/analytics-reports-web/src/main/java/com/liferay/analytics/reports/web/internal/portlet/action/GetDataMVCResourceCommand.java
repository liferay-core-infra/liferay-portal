...
public class GetDataMVCResourceCommand extends BaseMVCResourceCommand {
	...
	private JSONArray _getTimeSpansJSONArray(ResourceBundle resourceBundle) {
		List<TimeSpan> list = Arrays.asList(TimeSpan.values());

		list = ListUtil.filter(list, timeSpan -> timeSpan != TimeSpan.TODAY);

		list.sort(Comparator.comparingInt(TimeSpan::getDays));
