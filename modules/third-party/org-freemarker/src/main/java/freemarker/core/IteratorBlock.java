...
final class IteratorBlock extends TemplateElement {
	 ...
	 */
	IteratorBlock(
		Expression listedExp, String loopVar1Name, String loopVar2Name,
		TemplateElements childrenBeforeElse, boolean hashListing,
		boolean forEach) {
	...
	boolean acceptWithResult(Environment env)
		throws IOException, TemplateException {
		...
		return env.visitIteratorBlock(
			new IterationContext(listedValue, loopVar1Name, loopVar2Name));
	}
	...
	protected String dump(boolean canonical) {
		...
		if (forEach) {
			buf.append(
				_CoreStringUtils.toFTLTopLevelIdentifierReference(
					loopVar1Name));
			buf.append(" in ");
	...
	int getParameterCount() {
		return 1 + (loopVar1Name != null ? 1 : 0) +
			(loopVar2Name != null ? 1 : 0);
	}
	...
	class IterationContext implements LocalContext {
		...
		public IterationContext(
			TemplateModel listedValue, String loopVar1Name,
			String loopVar2Name) {
		...
		boolean accept(Environment env) throws IOException, TemplateException {
			return executeNestedContent(env, getChildBuffer());
		}

		void loopForItemsElement(
				Environment env, TemplateElement[] childBuffer,
		 ...
		 */
		private boolean executeNestedContent(
				Environment env, TemplateElement[] childBuffer)
			throws IOException, TemplateException {
		...
		private boolean executedNestedContentForCollOrSeqListing(
				Environment env, TemplateElement[] childBuffer)
			throws IOException, TemplateException {
			...
			if (listedValue instanceof TemplateCollectionModel) {
				...
				final TemplateSequenceModel seqModel =
					(TemplateSequenceModel)listedValue;
				...
				throw new NonSequenceOrCollectionException(
					...
					new _ErrorDescriptionBuilder(
						"The value you try to list is ",
						new _DelayedAOrAn(
							new _DelayedFTLTypeDescription(listedValue)),
						", thus you must specify two loop variables after the \"as\"; one for the key, and " +
							"another for the value, like ",
						"<#... as k, v>", ")."));
			}
			else {
				throw new NonSequenceOrCollectionException(
		...
		private boolean executedNestedContentForHashListing(
				Environment env, TemplateElement[] childBuffer)
			throws IOException, TemplateException {
			...
			if (listedValue instanceof TemplateHashModelEx) {
				TemplateHashModelEx listedHash =
					(TemplateHashModelEx)listedValue;

				if (listedHash instanceof TemplateHashModelEx2) {
					TemplateHashModelEx2.KeyValuePairIterator kvpIter =
						openedIterator == null ?
							((TemplateHashModelEx2)listedHash).
					...
					if (hashNotEmpty) {
						if (loopVar1Name != null) {
                            listLoop: do {
								TemplateHashModelEx2.KeyValuePair kvp =
									kvpIter.next();
					...
					TemplateModelIterator keysIter = listedHash.keys(
					).iterator();
					...
					if (hashNotEmpty) {
						if (loopVar1Name != null) {
                            listLoop: do {
								...
								loopVar2Value = listedHash.get(
									((TemplateScalarModel)loopVar1Value).
										getAsString());
				...
				throw new NonExtendedHashException(listedExp, listedValue, env);
			}
		...
		boolean hasVisibleLoopVar(String visibleLoopVarName) {
			...
			return visibleLoopVarName.equals(visibleLoopVar1Name) ||
				   visibleLoopVarName.equals(loopVar2Name);
		}
				...
				switch (name.length() - visibleLoopVar1Name.length()) {
					case 0:
						return loopVar1Value != null ? loopVar1Value :
							...
							).getFallbackOnNullLoopVariable() ? null :
					...
					case 9:
						if (name.endsWith(LOOP_STATE_HAS_NEXT)) {
							return hasNext ? TemplateBooleanModel.TRUE :
		...
		public Collection<String> getLocalVariableNames() {
			...
			if (visibleLoopVar1Name != null) {
				if (localVarNames == null) {
					...
					localVarNames.add(
						visibleLoopVar1Name + LOOP_STATE_HAS_NEXT);
				}
