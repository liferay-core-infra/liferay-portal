...
public class BatchEngineImportTaskPersistenceImpl
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByExecuteStatus = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByExecuteStatus",
			new String[] {
				String.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			},
		...
		_finderPaths.put(
			"finderPathWithPaginationFindByExecuteStatus",
			_finderPathWithPaginationFindByExecuteStatus);

		_finderPaths.put(
			"finderPathWithoutPaginationFindByExecuteStatus",
			_finderPathWithoutPaginationFindByExecuteStatus);

		_finderPaths.put(
			"finderPathCountByExecuteStatus", _finderPathCountByExecuteStatus);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<BatchEngineImportTask> batchEngineImportTasks = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<BatchEngineImportTask>> resultMap =
				new HashMap<>();
			...
			for (BatchEngineImportTask batchEngineImportTask :
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), batchEngineImportTask);
				}
				else {
					List<BatchEngineImportTask> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(batchEngineImportTask);
				}
			}

			for (Map.Entry<List<Object>, List<BatchEngineImportTask>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<BatchEngineImportTask> value = resultEntry.getValue();
