...
public class KBTemplatePersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<KBTemplate> kbTemplates = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<KBTemplate>> resultMap = new HashMap<>();
			...
			for (KBTemplate kbTemplate : kbTemplates) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						kbTemplateModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), kbTemplate);
				}
				else {
					List<KBTemplate> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(kbTemplate);
				}
			}

			for (Map.Entry<List<Object>, List<KBTemplate>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<KBTemplate> value = resultEntry.getValue();
