...
public class BrowserTrackerPersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<BrowserTracker> browserTrackers = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<BrowserTracker>> resultMap = new HashMap<>();
			...
			for (BrowserTracker browserTracker : browserTrackers) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						browserTrackerModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					FinderCacheUtil.putResult(
						finderPath, arguments.toArray(), browserTracker);
				}
				else {
					List<BrowserTracker> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(browserTracker);
				}
			}

			for (Map.Entry<List<Object>, List<BrowserTracker>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<BrowserTracker> value = resultEntry.getValue();
