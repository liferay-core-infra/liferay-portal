...
public class CommerceInventoryWarehouseItemPersistenceImpl
	...
	public CommerceInventoryWarehouseItem updateImpl(
		...
		if (Validator.isNull(
			...
			if (userId > 0) {
				long companyId = commerceInventoryWarehouseItem.getCompanyId();
				...
				if (!isNew) {
					classPK = commerceInventoryWarehouseItem.getPrimaryKey();
				}
				...
				try {
					commerceInventoryWarehouseItem.setExternalReferenceCode(
						SanitizerUtil.sanitize(
							...
							classPK, ContentTypes.TEXT_HTML, Sanitizer.MODE_ALL,
							commerceInventoryWarehouseItem.
								getExternalReferenceCode(),
							null));
				}
