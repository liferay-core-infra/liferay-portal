...
public class RedundantIndexEntryPersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<RedundantIndexEntry> redundantIndexEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<RedundantIndexEntry>> resultMap =
				new HashMap<>();

			for (RedundantIndexEntry redundantIndexEntry :
					redundantIndexEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						redundantIndexEntryModelImpl.getColumnValue(
							columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), redundantIndexEntry);
				}
				else {
					List<RedundantIndexEntry> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(redundantIndexEntry);
				}
			}

			for (Map.Entry<List<Object>, List<RedundantIndexEntry>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<RedundantIndexEntry> value = resultEntry.getValue();
