...
public class DatabasePoolSizeAndTomcatThreadsRuleImplTest {
	...
	@Test
	public void testDatabasePoolSizeAndTomcatThreadsRuleCheckReturnsResult()
		throws Exception {

		BundleContext bundleContext = _getBundleContext();

		ServiceReference<ProductionReadinessRule> serviceReference =
			_getDatabasePoolSizeAndTomcatThreadsRuleServiceReference(
				bundleContext);
		...
		try {
			...
			Assert.assertTrue(results.toString(), results.size() <= 1);
	...
	@Test
	public void testDatabasePoolSizeAndTomcatThreadsRuleIsRegistered()
		throws Exception {
	...
	private BundleContext _getBundleContext() {
		return FrameworkUtil.getBundle(
			DatabasePoolSizeAndTomcatThreadsRuleImplTest.class
		).getBundleContext();
	}
	...
	private static final String _RULE_KEY = "pool-vs-thread-ratio";
