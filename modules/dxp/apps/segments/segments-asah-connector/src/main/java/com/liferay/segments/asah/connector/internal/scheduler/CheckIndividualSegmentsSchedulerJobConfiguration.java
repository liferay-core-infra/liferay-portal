...
public class CheckIndividualSegmentsSchedulerJobConfiguration
	...
	private void _checkIndividualSegments() throws Exception {
		_individualSegmentsChecker._checkIndividualSegments();
		_individualSegmentsChecker._checkIndividualSegmentsMemberships();
	}
	...
	private final IndividualSegmentsChecker _individualSegmentsChecker =
		new IndividualSegmentsChecker();
	...
	private class IndividualSegmentsChecker {
			...
			try {
				...
				if (segmentsEntry == null) {
					_segmentsEntryLocalService.addSegmentsEntry(
						individualSegment.getId(), nameMap,
		...
		private void _addSegmentsEntryRels(
			...
			try {
				...
				_log.error(
					"Unable to process individuals " + userIds,
					portalException);
			}
		}
				...
				while (true) {
					...
					individualResults =
						_asahFaroBackendClient.getIndividualResults(
							segmentsEntry.getCompanyId(),
							segmentsEntry.getSegmentsEntryKey(), curPage,
							_DELTA,
							Collections.singletonList(
								OrderByField.desc("dateModified")));
				}
