...
public class DepotEntryPersistenceImpl
	...
	public void activate() {
		...
		_finderPathFetchByGroupId = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByGroupId",
		...
		_finderPaths.put("finderPathFetchByGroupId", _finderPathFetchByGroupId);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<DepotEntry> depotEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<DepotEntry>> resultMap = new HashMap<>();

			for (DepotEntry depotEntry : depotEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						depotEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), depotEntry);
				}
				else {
					List<DepotEntry> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(depotEntry);
				}
			}

			for (Map.Entry<List<Object>, List<DepotEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<DepotEntry> value = resultEntry.getValue();
