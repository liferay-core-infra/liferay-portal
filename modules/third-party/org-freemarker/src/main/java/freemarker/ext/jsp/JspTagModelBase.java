...
class JspTagModelBase {
	...
	protected final TemplateModelException toTemplateModelExceptionOrRethrow(
			Exception e)
		throws TemplateModelException {

		if ((e instanceof RuntimeException) &&
			!isCommonRuntimeException((RuntimeException)e)) {
		...
		if (e instanceof TemplateExceptionWrapperJspException) {
			return toTemplateModelExceptionOrRethrow(
				((TemplateExceptionWrapperJspException)e).getCause());
		}
	...
	private boolean isCommonRuntimeException(RuntimeException e) {
		...
		if ((eClass == NullPointerException.class) ||
	...
	void setupTag(Object tag, Map args, ObjectWrapper wrapper)
		...
		if ((args != null) && !args.isEmpty()) {
			ObjectWrapperAndUnwrapper unwrapper =
				wrapper instanceof ObjectWrapperAndUnwrapper ?
			...
			for (Iterator iter = args.entrySet(
				...
				if (setterMethod == null) {
					...
					try {
						...
						final Class<?> setterType =
							setterMethod.getParameterTypes()[0];
						_ErrorDescriptionBuilder desc =
							new _ErrorDescriptionBuilder(
								"Failed to set JSP tag parameter ",
								new _DelayedJQuote(paramName),
								" (declared type: ",
								new _DelayedShortClassName(setterType) +
									", actual value's type: ",
								argArray[0] != null ?
									new _DelayedShortClassName(
										argArray[0].getClass()) : "Null",
								"). See cause exception for the more specific cause...");

						if ((e instanceof IllegalArgumentException) &&
							!setterType.isAssignableFrom(String.class) &&
							(argArray[0] != null) &&
							(argArray[0] instanceof String)) {
