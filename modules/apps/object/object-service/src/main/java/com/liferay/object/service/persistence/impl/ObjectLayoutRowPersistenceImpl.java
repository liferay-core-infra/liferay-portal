...
public class ObjectLayoutRowPersistenceImpl
	...
	public void activate() {
		...
		_finderPaths.put(
			"finderPathWithPaginationFindByObjectLayoutBoxId",
			_finderPathWithPaginationFindByObjectLayoutBoxId);

		_finderPaths.put(
			"finderPathWithoutPaginationFindByObjectLayoutBoxId",
			_finderPathWithoutPaginationFindByObjectLayoutBoxId);

		_finderPaths.put(
			"finderPathCountByObjectLayoutBoxId",
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<ObjectLayoutRow> objectLayoutRows = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<ObjectLayoutRow>> resultMap =
				new HashMap<>();
			...
			for (ObjectLayoutRow objectLayoutRow : objectLayoutRows) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						objectLayoutRowModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), objectLayoutRow);
				}
				else {
					List<ObjectLayoutRow> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(objectLayoutRow);
				}
			}

			for (Map.Entry<List<Object>, List<ObjectLayoutRow>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<ObjectLayoutRow> value = resultEntry.getValue();
