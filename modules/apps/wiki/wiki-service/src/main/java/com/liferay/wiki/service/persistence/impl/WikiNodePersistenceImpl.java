...
public class WikiNodePersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<WikiNode> wikiNodes = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<WikiNode>> resultMap = new HashMap<>();
			...
			for (WikiNode wikiNode : wikiNodes) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					WikiNodeModelImpl wikiNodeModelImpl =
						(WikiNodeModelImpl)wikiNode;

					arguments.add(wikiNodeModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), wikiNode);
				}
				else {
					List<WikiNode> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(wikiNode);
				}
			}

			for (Map.Entry<List<Object>, List<WikiNode>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<WikiNode> value = resultEntry.getValue();
