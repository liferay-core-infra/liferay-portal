...
public class ObjectStatePersistenceImpl
	...
	public void activate() {
		...
		_finderPathWithoutPaginationFindByListTypeEntryId = new FinderPath(
			...
			"findByListTypeEntryId", new String[] {Long.class.getName()},
			new String[] {"listTypeEntryId"}, true);

		_finderPathCountByListTypeEntryId = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByListTypeEntryId", new String[] {Long.class.getName()},
		...
		_finderPathFetchByLTEI_OSFI = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByLTEI_OSFI",
			new String[] {Long.class.getName(), Long.class.getName()},
			new String[] {"listTypeEntryId", "objectStateFlowId"}, true);

		_finderPathCountByLTEI_OSFI = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByLTEI_OSFI",
			new String[] {Long.class.getName(), Long.class.getName()},
		...
		_finderPaths.put(
			"finderPathFetchByLTEI_OSFI", _finderPathFetchByLTEI_OSFI);

		_finderPaths.put(
			"finderPathCountByLTEI_OSFI", _finderPathCountByLTEI_OSFI);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<ObjectState> objectStates = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<ObjectState>> resultMap = new HashMap<>();
			...
			for (ObjectState objectState : objectStates) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						objectStateModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), objectState);
				}
				else {
					List<ObjectState> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(objectState);
				}
			}

			for (Map.Entry<List<Object>, List<ObjectState>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<ObjectState> value = resultEntry.getValue();
