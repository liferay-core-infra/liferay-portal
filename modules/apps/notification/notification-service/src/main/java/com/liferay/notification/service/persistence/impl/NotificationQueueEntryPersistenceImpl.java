...
public class NotificationQueueEntryPersistenceImpl
	 ...
	 */
	@Override
	public List<NotificationQueueEntry> filterFindBySent(boolean sent) {
		return filterFindBySent(
	 ...
	 */
	@Override
	public List<NotificationQueueEntry> filterFindBySent(
		boolean sent, int start, int end) {

		return filterFindBySent(sent, start, end, null);
	}
	...
	public List<NotificationQueueEntry> filterFindBySent(
		...
		if (!InlineSQLHelperUtil.isEnabled()) {
			return findBySent(sent, start, end, orderByComparator);
		}
	 ...
	 */
	@Override
	public NotificationQueueEntry[] filterFindBySent_PrevAndNext(
			long notificationQueueEntryId, boolean sent,
			OrderByComparator<NotificationQueueEntry> orderByComparator)
		throws NoSuchNotificationQueueEntryException {

		if (!InlineSQLHelperUtil.isEnabled()) {
			return findBySent_PrevAndNext(
		...
		try {
			...
			array[0] = filterGetBySent_PrevAndNext(
				session, notificationQueueEntry, sent, orderByComparator, true);

			array[1] = notificationQueueEntry;

			array[2] = filterGetBySent_PrevAndNext(
				session, notificationQueueEntry, sent, orderByComparator,
				false);
	...
	protected NotificationQueueEntry filterGetBySent_PrevAndNext(
		Session session, NotificationQueueEntry notificationQueueEntry,
		boolean sent,
		OrderByComparator<NotificationQueueEntry> orderByComparator,
		boolean previous) {
	 ...
	 */
	@Override
	public int filterCountBySent(boolean sent) {
		if (!InlineSQLHelperUtil.isEnabled()) {
			return countBySent(sent);
		}
	...
	public void activate() {
		...
		_finderPathWithPaginationFindBySent = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findBySent",
			new String[] {
		...
		_finderPathWithoutPaginationFindBySent = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findBySent",
			new String[] {Boolean.class.getName()}, new String[] {"sent"},
			true);

		_finderPathCountBySent = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countBySent",
			new String[] {Boolean.class.getName()}, new String[] {"sent"},
			false);

		_finderPathWithPaginationFindByLtSentDate = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByLtSentDate",
			new String[] {
		...
		_finderPathWithPaginationCountByLtSentDate = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "countByLtSentDate",
