...
public class ObjectViewColumnPersistenceImpl
	...
	public void activate() {
		...
		_finderPathWithPaginationFindByObjectViewId = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByObjectViewId",
			new String[] {
		...
		_finderPathWithPaginationFindByOVI_OFN = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByOVI_OFN",
			new String[] {
		...
		_finderPathWithoutPaginationFindByOVI_OFN = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByOVI_OFN",
			new String[] {Long.class.getName(), String.class.getName()},
			new String[] {"objectViewId", "objectFieldName"}, true);

		_finderPathCountByOVI_OFN = new FinderPath(
			this, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByOVI_OFN",
			new String[] {Long.class.getName(), String.class.getName()},
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<ObjectViewColumn> objectViewColumns = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<ObjectViewColumn>> resultMap =
				new HashMap<>();
			...
			for (ObjectViewColumn objectViewColumn : objectViewColumns) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						objectViewColumnModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), objectViewColumn);
				}
				else {
					List<ObjectViewColumn> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(objectViewColumn);
				}
			}

			for (Map.Entry<List<Object>, List<ObjectViewColumn>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<ObjectViewColumn> value = resultEntry.getValue();
