...
public class ERCCompanyEntryPersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<ERCCompanyEntry> ercCompanyEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<ERCCompanyEntry>> resultMap =
				new HashMap<>();

			for (ERCCompanyEntry ercCompanyEntry : ercCompanyEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						ercCompanyEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), ercCompanyEntry);
				}
				else {
					List<ERCCompanyEntry> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(ercCompanyEntry);
				}
			}

			for (Map.Entry<List<Object>, List<ERCCompanyEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<ERCCompanyEntry> value = resultEntry.getValue();
