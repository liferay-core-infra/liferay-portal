/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.headless.commerce.delivery.cart.internal.dto.v1_0.constants;

/**
 * @author Renan Vasconcelos
 */
public class DTOConverterConstants {

	public static final String ADDRESS_DTO_CONVERTER =
		"(component.name=com.liferay.headless.commerce.delivery.cart." +
			"internal.dto.v1_0.AddressDTOConverter)";

	public static final String CART_DTO_CONVERTER =
		"(component.name=com.liferay.headless.commerce.delivery.cart." +
			"internal.dto.v1_0.CartDTOConverter)";

	public static final String CART_ITEM_DTO_CONVERTER =
		"(component.name=com.liferay.headless.commerce.delivery.cart." +
			"internal.dto.v1_0.CartItemDTOConverter)";

	public static final String NOTE_DTO_CONVERTER =
		"(component.name=com.liferay.headless.commerce.delivery.cart." +
			"internal.dto.v1_0.NoteDTOConverter)";

}