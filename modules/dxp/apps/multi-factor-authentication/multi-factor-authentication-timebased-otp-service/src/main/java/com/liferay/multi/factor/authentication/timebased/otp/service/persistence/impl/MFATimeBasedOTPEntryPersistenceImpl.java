...
public class MFATimeBasedOTPEntryPersistenceImpl
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<MFATimeBasedOTPEntry> mfaTimeBasedOTPEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<MFATimeBasedOTPEntry>> resultMap =
				new HashMap<>();

			for (MFATimeBasedOTPEntry mfaTimeBasedOTPEntry :
					mfaTimeBasedOTPEntrys) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						mfaTimeBasedOTPEntryModelImpl.getColumnValue(
							columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), mfaTimeBasedOTPEntry);
				}
				else {
					List<MFATimeBasedOTPEntry> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(mfaTimeBasedOTPEntry);
				}
			}

			for (Map.Entry<List<Object>, List<MFATimeBasedOTPEntry>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<MFATimeBasedOTPEntry> value = resultEntry.getValue();
