...
public class KaleoProcessPersistenceImpl
	...
	public void activate() {
		...
		_finderPathFetchByDDLRecordSetId = new FinderPath(
			this, FINDER_CLASS_NAME_ENTITY, "fetchByDDLRecordSetId",
		...
		_finderPathCountByDDLRecordSetId = new FinderPath(
			...
			"countByDDLRecordSetId", new String[] {Long.class.getName()},
		...
		_finderPaths.put(
			"finderPathFetchByDDLRecordSetId",
			_finderPathFetchByDDLRecordSetId);

		_finderPaths.put(
			"finderPathCountByDDLRecordSetId",
			_finderPathCountByDDLRecordSetId);
	...
	public void populateFinderCache(FinderPath... finderPaths) {
		List<KaleoProcess> kaleoProcesss = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<KaleoProcess>> resultMap = new HashMap<>();

			for (KaleoProcess kaleoProcess : kaleoProcesss) {
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						kaleoProcessModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), kaleoProcess);
				}
				else {
					List<KaleoProcess> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(kaleoProcess);
				}
			}

			for (Map.Entry<List<Object>, List<KaleoProcess>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<KaleoProcess> value = resultEntry.getValue();
