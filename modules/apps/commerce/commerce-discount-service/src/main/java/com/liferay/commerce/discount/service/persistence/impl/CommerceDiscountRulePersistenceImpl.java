 ...
 */
@Component(
	service = {CommerceDiscountRulePersistence.class, BasePersistence.class}
)
public class CommerceDiscountRulePersistenceImpl
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<CommerceDiscountRule> commerceDiscountRules = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<CommerceDiscountRule>> resultMap =
				new HashMap<>();
			...
			for (CommerceDiscountRule commerceDiscountRule :
				...
				for (String columnName : finderPath.getColumnNames()) {
					...
					arguments.add(
						commerceDiscountRuleModelImpl.getColumnValue(
							columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), commerceDiscountRule);
				}
				else {
					List<CommerceDiscountRule> resultList =
						resultMap.computeIfAbsent(
							arguments, key -> new ArrayList<>());

					resultList.add(commerceDiscountRule);
				}
			}

			for (Map.Entry<List<Object>, List<CommerceDiscountRule>>
					resultEntry : resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<CommerceDiscountRule> value = resultEntry.getValue();
