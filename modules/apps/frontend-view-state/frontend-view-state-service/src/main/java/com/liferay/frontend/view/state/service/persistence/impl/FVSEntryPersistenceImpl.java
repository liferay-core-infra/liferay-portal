...
public class FVSEntryPersistenceImpl
	...
	public void activate() {
		...
		_setFVSEntryUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setFVSEntryUtilPersistence(null);
	...
	public void loadFinderCache(FinderPath[] finderPaths) {
		...
		List<FVSEntry> fvsEntrys = findAll();

		for (FinderPath finderPath : finderPaths) {
			Map<List<Object>, List<FVSEntry>> resultMap = new HashMap<>();

			for (FVSEntry fvsEntry : fvsEntrys) {
				List<Object> arguments = new ArrayList<>();

				for (String columnName : finderPath.getColumnNames()) {
					FVSEntryModelImpl fvsEntryModelImpl =
						(FVSEntryModelImpl)fvsEntry;

					arguments.add(fvsEntryModelImpl.getColumnValue(columnName));
				}
				...
				if (Objects.equals(
					...
					finderCache.putResult(
						finderPath, arguments.toArray(), fvsEntry);
				}
				else {
					List<FVSEntry> resultList = resultMap.computeIfAbsent(
						arguments, key -> new ArrayList<>());

					resultList.add(fvsEntry);
				}
			}

			for (Map.Entry<List<Object>, List<FVSEntry>> resultEntry :
					resultMap.entrySet()) {

				List<Object> key = resultEntry.getKey();
				List<FVSEntry> value = resultEntry.getValue();
	...
	private void _setFVSEntryUtilPersistence(
		FVSEntryPersistence fvsEntryPersistence) {

		try {
			Field field = FVSEntryUtil.class.getDeclaredField("_persistence");

			field.setAccessible(true);

			field.set(null, fvsEntryPersistence);
		}
